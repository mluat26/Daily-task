...
const App: React.FC = () => {
  const [projects, setProjects] = useState<Project[]>(() => {
    const saved = localStorage.getItem('ff_projects_v6');
  ...
  useEffect(() => {
    localStorage.setItem('ff_projects_v6', JSON.stringify(projects));
  }, [projects]);
  ...
  return (
    <div className={`flex flex-col md:flex-row min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-[#F8FAFC] text-slate-900'}`}>
      <aside className={`w-full md:w-72 border-r p-6 flex flex-col gap-8 transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl shadow-lg text-white ${isDarkMode ? 'bg-indigo-500 shadow-indigo-900/20' : 'bg-slate-900 shadow-slate-200'}`}>
              <Sparkles size={24} />
            </div>
            <h1 className="text-2xl font-black tracking-tight italic">FreeFlow.</h1>
          </div>
          <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-xl transition-colors ${isDarkMode ? 'bg-slate-800 text-amber-400 hover:bg-slate-700' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
        ...
        <div className="mt-auto">
          <div className={`p-6 rounded-[2rem] text-white shadow-xl ${isDarkMode ? 'bg-slate-800 shadow-none border border-slate-700' : 'bg-slate-900 shadow-slate-200'}`}>
            ...
            <p className="text-sm font-medium leading-relaxed mb-4 text-slate-300">Tư vấn ưu tiên dự án & dòng tiền.</p>
            <button onClick={async () => { setIsAiLoading(true); const advice = await getWorkloadAdvice(projects); setAiAdvice(advice); setIsAiLoading(false); }} disabled={isAiLoading} className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2">{isAiLoading ? <Loader2 className="animate-spin w-3 h-3" /> : 'Phân tích'}</button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto max-w-6xl mx-auto w-full">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-500 font-medium">Đang có {stats.activeCount} dự án active.</p>
          </div>
          <button onClick={() => setIsAddingProject(true)} className="group flex items-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold transition-all shadow-xl shadow-indigo-500/20 active:scale-95"><Plus size={20} className="group-hover:rotate-90 transition-transform" />Dự án mới</button>
        </header>

        {aiAdvice && <div className={`mb-10 p-6 border shadow-sm rounded-[2rem] relative overflow-hidden group transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}><div className="flex gap-5"><div className="bg-indigo-600 p-3.5 rounded-2xl text-white h-fit shadow-lg shadow-indigo-500/20"><BrainCircuit size={24} /></div><div className="flex-1"><h3 className="text-lg font-bold mb-2">Lời khuyên từ Gemini</h3><p className={`text-sm leading-relaxed whitespace-pre-line mb-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{aiAdvice}</p><button onClick={() => setAiAdvice('')} className="text-xs font-bold text-indigo-500 hover:text-indigo-400 uppercase tracking-widest">Đã ghi nhận</button></div></div></div>}
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className={`lg:col-span-2 p-8 rounded-[2.5rem] border shadow-sm transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                ...
                <div className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDarkMode ? '#1E293B' : '#F1F5F9'} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#94A3B8', fontSize: 12}} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{fill: '#94A3B8', fontSize: 12}} /><Tooltip cursor={{fill: isDarkMode ? '#1E293B' : '#F8FAFC'}} contentStyle={{borderRadius: '16px', border: 'none', backgroundColor: isDarkMode ? '#1E293B' : '#FFFFFF', color: isDarkMode ? '#F8FAFC' : '#0F172A', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}} formatter={(val: number) => [`${val}M VNĐ`, 'Budget']} /><Bar dataKey="budget" radius={[8, 8, 0, 0]} barSize={45}>{chartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.progress === 100 ? '#10B981' : '#6366F1'} />))}</Bar></BarChart></ResponsiveContainer></div>
              </div>
        ...
        {activeTab === 'finance' && (
          <div className={`rounded-[2.5rem] border shadow-sm overflow-hidden transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
            ...
            <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className={`text-[11px] font-black text-slate-400 uppercase tracking-widest ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}><th className="px-10 py-6">Dự án</th><th className="px-10 py-6">Budget</th><th className="px-10 py-6 text-center">Trạng thái</th><th className="px-10 py-6 text-right">Ngày nhận</th></tr></thead><tbody className={`divide-y ${isDarkMode ? 'divide-slate-800' : 'divide-slate-50'}`}>{projects.map(p => (<tr key={p.id} className={`transition-colors ${isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50/50'}`}><td className="px-10 py-6 font-bold">{p.projectName}</td><td className="px-10 py-6 font-mono font-bold">{formatVND(p.budget)}</td><td className="px-10 py-6 text-center"><button onClick={() => updateProject(p.id, { paymentStatus: p.paymentStatus === PaymentStatus.PAID ? PaymentStatus.PENDING : PaymentStatus.PAID })} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 ${p.paymentStatus === PaymentStatus.PAID ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>{p.paymentStatus}</button></td><td className="px-10 py-6 text-sm text-slate-400 font-medium text-right">{new Date(p.createdAt).toLocaleDateString('vi-VN')}</td></tr>))}</tbody></table></div>
          </div>
        )}
      </main>
          ...
          <div className={`rounded-[3rem] w-full max-w-2xl p-12 shadow-2xl animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-slate-900 text-slate-100 border border-slate-800' : 'bg-white text-slate-900'}`}>
            ...
            <form onSubmit={handleCreateProject} className="space-y-6">
              ...
              {creationMode === 'ai' ? (
                ...
                <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Mô tả dự án (Tùy chọn)</label><textarea name="description" className={`w-full px-6 py-4 rounded-2xl outline-none border-2 transition-all font-medium h-24 ${isDarkMode ? 'bg-slate-800 border-slate-800 focus:border-indigo-500' : 'bg-slate-50 border-slate-50 focus:border-indigo-500'}`} placeholder="Chi tiết ngắn gọn..." /></div>
              )}
...
const ProjectCard: React.FC<{ 
  ...
  return (
    <div className={`rounded-[2.5rem] border p-8 flex flex-col hover:shadow-2xl transition-all group relative ${isDarkMode ? 'bg-slate-900 hover:shadow-indigo-500/5' : 'bg-white hover:shadow-slate-100'} ${project.isUrgent ? 'border-orange-500/30' : (isDarkMode ? 'border-slate-800' : 'border-slate-100')}`}>
      {project.isUrgent && <div className="absolute -top-3 left-8 bg-orange-600 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-orange-500/20 animate-pulse flex items-center gap-1"><Flame size={12} /> GẤP</div>}
      
      <div className="flex justify-between items-start mb-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <select value={project.status} onChange={(e) => onUpdateProject({ status: e.target.value as ProjectStatus })} className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest outline-none border-none cursor-pointer ${project.status === ProjectStatus.COMPLETED ? 'bg-emerald-500/10 text-emerald-500' : project.status === ProjectStatus.IN_PROGRESS ? 'bg-indigo-500/10 text-indigo-500' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-600')}`}>{Object.values(ProjectStatus).map(s => <option key={s} value={s}>{s}</option>)}</select>
          </div>
      ...
      <div className={`pt-8 border-t mt-auto flex justify-between items-end ${isDarkMode ? 'border-slate-800' : 'border-slate-50'}`}>
        <div>
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Hạn dự án</p>
